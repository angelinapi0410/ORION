...
(function ($) {
  ...
  jQuery(document).ready(function ($) {
    $(".ct-p-t-s ul li").on("click", function () {
      $(".ct-p-t-s ul li").removeClass("active");
      $(this).addClass("active");
      var isYear = $(this).index() === 1;
      $(".ct-pricing-card2").each(function () {
        var $priceSpans = $(this).find("h2.price-t span");
        if (isYear) {
          $priceSpans.removeClass("active");
          $priceSpans.eq(1).addClass("active");
        } else {
          $priceSpans.removeClass("active");
          $priceSpans.eq(0).addClass("active");
        }
      });
    });
  });
...
document
  .querySelectorAll(".ct-hero-section5 .content h2.highlight span")
  .forEach((element) => {
    const corners = ["tl", "tr", "bl", "br"];

    corners.forEach((corner) => {
      const span = document.createElement("span");
      span.classList.add(`corner-${corner}`);
